 ...
 */
public class FragmentRenewal extends javax.swing.JPanel {

    private ArrayList<EnrollmentDetails> renewApplication;

    /**
     * Creates new form FragmentRenewal
     */
    public FragmentRenewal() {
        initComponents();
        init();
    }
    ...
    private void init() {
        ...
        renewApplication = new ArrayList<>();
        renewApplication = new CrudBean().getRenewalApplications();
        if (renewApplication.size() > 0) {
            for (int i = 0; i < renewApplication.size(); i++) {
                EnrollmentDetails details = renewApplication.get(i);
                tableSwing91.addRow(new Object[]{details.getPoliceId(), details.getFirstName(), details.getSurname(), details.getPosition(),
                    details.getRank(), details.getCounty(), details.getState(), (details.getStatus().equals("1") ? StatusType.Renew : StatusType.Reset)});
    ...
    private void tableSwing91MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableSwing91MouseClicked
        ...
        int value = (value1.equals("Renew") ? 1 : 0);

        if (value1.equals("Renew")) {
