 ...
 */
public class FragmentReissue extends javax.swing.JPanel {

    private ArrayList<EnrollmentDetails> reissueApplication;

    /**
     * Creates new form FragmentReissue
     */
    public FragmentReissue() {
        initComponents();
    ...
    private void init() {
        ...
        reissueApplication = new ArrayList<>();
        reissueApplication = new CrudBean().getReissueApplications();
        if (reissueApplication.size() > 0) {
            for (int i = 0; i < reissueApplication.size(); i++) {
                EnrollmentDetails details = reissueApplication.get(i);
                tableSwing91.addRow(new Object[]{details.getPoliceId(), details.getFirstName(), details.getSurname(), details.getPosition(),
                    details.getRank(), details.getCounty(), details.getState(), (details.getStatus().equals("1") ? StatusType.Reissue : StatusType.Reset)});
    ...
    private void tableSwing91MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableSwing91MouseClicked
        ...
        int value = (value1.equals("Reissue") ? 1 : 0);

        if (value1.equals("Reissue")) {
