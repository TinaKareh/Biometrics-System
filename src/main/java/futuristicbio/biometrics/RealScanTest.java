...
public class RealScanTest extends javax.swing.JFrame {
    ...
    public Realscan_Fingerprints fingerprint;
    /**
     * Creates new form RealScanTest
     * @param fingerprint
     */
    public RealScanTest(Realscan_Fingerprints fingerprint) {
        initComponents();
    ...
    private void initComponents() {
        ...
        jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder("LFD Option (RS-G1, D, G10)"));

        jLabel9.setText("LFD Level");

        LFDLevel.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "OFF", "Level 1", "Level 2", "Level 3", "Level 4", "Level 5" }));
        ...
        jPanel3.getAccessibleContext().setAccessibleName("LFD Option (RS-G1)");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                RealScanTest test = new RealScanTest(null);
    ...
    public void saveImageFile(byte[] imgByte, int imageWidth, int imageHeight) throws IOException {
        int result;
        if (selectResult.getSelectedIndex() == 1) // save image
        {
            ...
            if (selected == javax.swing.JFileChooser.APPROVE_OPTION) {
                ...
                fingerprint.displayfingerprints();
    ...
    //
    public void RSRawCaptureResultCallback( int deviceHandle, int errorCode, byte imageData[], int imageWidth, int imageHeight ) throws IOException
    {
        try {
            Thread.sleep(1);
        ...
        if(LFDLevel.getSelectedIndex() == 0)
        {
            statusField.setText("An image is captured successfully");
        }
        else
        {
            RealScan_JNI.RSLFDResult sLFDResult = new RealScan_JNI.RSLFDResult();
            /*
            for(int i=0; i<RealScan_JNI.RS_LFD_MAX_INFO; i++)
            {
                sLFDResult.sLFDInfo[i] = new RealScan_JNI.RSLFDInfo2();
            }
            */
            
            int nRetVal = RealScan_JNI.RS_GetLFDResult(deviceHandle, sLFDResult);
            if(nRetVal != RealScan_JNI.RS_SUCCESS)
            {
                statusField.setText("An image is captured successfully");
            }
            else
            {              
                ...
                int nFinalLFDResult = RealScan_JNI.RS_LFD_LIVE;
                String strFinalMsg = "";
                String strSocres = "[";
                for(int i=0; i<sLFDResult.nNumofFinger; i++)
                {
                    String strScore;
                    if(sLFDResult.nResult[i] == RealScan_JNI.RS_LFD_FAKE)
                    {
                        nFinalLFDResult = RealScan_JNI.RS_LFD_FAKE;
                        strScore = "F" + "(" + sLFDResult.nScore[i] + ")";
                    }
                    else
                    {
                        strScore = "L" + "(" + sLFDResult.nScore[i] + ")";
                    }
                    strSocres = strSocres + strScore;                   
                }
                if(nFinalLFDResult == RealScan_JNI.RS_LFD_LIVE)
                {
                    strFinalMsg = "[LIVE FINGER]" + strSocres + "]";
                }
                else
                {
                    strFinalMsg = "[FAKE FINGER]" + strSocres + "]";
                }
                statusField.setText(strFinalMsg);
            }
